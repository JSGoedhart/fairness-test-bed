...
class LFR(Transformer):
    ...
    def __init__(self,
        ...
        # Joosje
        self.path_loss = r'C:/Users/joosj/Documents/Master AI/Thesis/Code/AIF360/evaluations_joosje/results/losses.txt'
        self.path_confusion_matrix_sens = r'C:/Users/joosj/Documents/Master AI/Thesis/Code/AIF360/evaluations_joosje/results/confusion_matrix.txt'
    ...
    def fit(self, dataset, **kwargs):
        ...
        # Create arrays with sensitive features and labels, same for nonsensitive
        ...
        # Joosje: empty results 
        open(self.path_loss, 'w').close()
        open(self.path_confusion_matrix_sens, 'w').close()
        
        # Joosje: write first column of results
        f = open(self.path_loss, 'a+')
        f.write('L_x, L_y, L_z' + "\n")
        f.close()
        f = open(self.path_confusion_matrix_sens, 'a+')
        f.write('tn, fp, fn, tp' + "\n")
        f.close()

        fmin_l_bfgs = optim.fmin_l_bfgs_b(lfr_helpers.LFR_optim_obj, x0=model_inits, epsilon=1e-5,
                                  args=(training_sensitive, training_nonsensitive,
                                        ytrain_sensitive[:, 0], ytrain_nonsensitive[:, 0], self.k, self.Ax,
                                        self.Ay, self.Az, 0, self.print_interval, self.path_loss, 
                                        self.path_confusion_matrix_sens),
                                  bounds=bnd, approx_grad=True, maxfun=5000,
                                  maxiter=200, disp=self.verbose)

        self.learned_model = fmin_l_bfgs[0]
        print("Dictionairy", fmin_l_bfgs[2])
        print("Learned_model: ", self.learned_model)
        print('Initialized model: ', model_inits)
    ...
    def transform(self, dataset, threshold=0.5, **kwargs):
        ...
        print("M_nks")
        print(M_nk_sensitive[0:10])
        print(M_nk_nonsensitive[0:10])
        print("\n")
        ...
        # dataset_new.features = transformed_features # Joosje
