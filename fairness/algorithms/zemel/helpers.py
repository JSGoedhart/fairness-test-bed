...
def LFR_optim_obj(params, data_sensitive, data_nonsensitive, y_sensitive,
                  ...
                  path_loss = None, path_conf_m_s=None):
    
    # Joosje: arrays to keep track of losses and FP/ TP/ TN/ FN rates per group
    result_arrs = []
    ...
    if (LFR_optim_obj.iters - 1) % print_inteval == 0:
        print(LFR_optim_obj.iters, criterion)
        
        # Joosje
        # print("A_x = {A_x} and A_y = {A_y} and A_z = {A_z}".format(A_x = A_x, A_y = A_y, A_z= A_z))
        # print("L_x = {L_x} and L_y = {L_y} and L_z = {L_z}".format(L_x = L_x, L_y = L_y, L_z= L_z))
        
        f = open(path_loss, 'a+')
        f.write(str([L_x, L_y, L_z]) + "\n")
        tn, fp, fn, tp = confusion_matrix(y_sensitive, np.around(yhat_sensitive)).ravel()
        f = open(path_conf_m_s, 'a+')
        f.write(str([tn, fp, fn, tp]) + "\n")
        tn, fp, fn, tp = confusion_matrix(y_nonsensitive, np.around(yhat_nonsensitive)).ravel()
        f.write(str([tn, fp, fn, tp]) + "\n")
        f.write("\n")
        # print(confusion_matrix(y_sensitive, np.around(yhat_sensitive)))
        # print(confusion_matrix(y_nonsensitive, np.around(yhat_nonsensitive)))
